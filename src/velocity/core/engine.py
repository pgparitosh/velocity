...
class AgentEngine:
            ...
            while not check_stop_conditions(ctx, conditions):
                ...
                if response.stop_reason == "tool_use":
                    ...
                    for idx, res in enumerate(results):
                        ...
                        content: str = str(res) if not isinstance(res, BaseException) else f"Tool execution failed: {res}"
    ...
    async def _execute_single_tool(self, agent: AgentBase, tool_call: dict[str, Any], ctx: AgentContext) -> str:
        ...
        try:
            ...
            elapsed_ms = (time.monotonic() - start_t) * 1000.0
            error_msg = str(err)
            ctx.record_tool_call(name, success=False, latency_ms=elapsed_ms, error_message=error_msg)
