...
class LlmResponse:
    ...
    __slots__ = ("content", "stop_reason", "input_tokens", "output_tokens", "tool_calls", "cost_usd")
...
class LLMGateway:
        ...
        for current_provider_name in providers_to_try:
            ...
            for attempt in range(self.max_retries + 1):
                if not breaker.allow_request():
                    logger.warning(f"Circuit open for provider {current_provider_name}. Failing fast.")
                ...
                try:
                    ...
                    logger.warning(
                        f"LLM Provider '{current_provider_name}' error: {e}. "
    ...
    async def call(
        ...
        response: LlmResponse = await self._execute_with_resilience(
            _do_call, target_provider
        )
